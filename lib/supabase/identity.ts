...
function registrationError(message?: string) {
  ...
  return new IdentityError(503, 'Não foi possível criar a conta agora. Tente novamente em alguns instantes.');
}
...
export async function registerMaster(input: Record<string, unknown>) {
  ...
  if (created.error || !created.data.user) throw registrationError(created.error?.message);
