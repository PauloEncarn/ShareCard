...
function registrationError(message?: string) {
  ...
  if (normalized.includes('already registered') || normalized.includes('already been registered') || normalized.includes('already exists')) {
    return new IdentityError(409, 'Este e-mail jÃ¡ possui uma conta. Entre ou use outro e-mail.');
  }
  ...
  if (normalized.includes('email')) return new IdentityError(400, 'Informe um e-mail vÃ¡lido.');
...
export async function registerMaster(input: Record<string, unknown>) {
  if (input.inviteToken) throw new IdentityError(400, 'O cadastro por convite serÃ¡ liberado na prÃ³xima etapa.');
  const name = requiredText(input.name, 'Nome', 80);
  if (!validEmail(input.email)) throw new IdentityError(400, 'Informe um e-mail vÃ¡lido.');
  ...
  if (inserted.error || !inserted.data) {
    ...
    throw new IdentityError(503, 'A conta foi criada, mas o espaÃ§o nÃ£o pÃ´de ser preparado. Tente novamente.');
  }
  return signIn(email, input.password);
}

export async function signIn(rawEmail: unknown, rawPassword: unknown) {
  if (!validEmail(rawEmail)) throw new IdentityError(400, 'Informe um e-mail vÃ¡lido.');
...
export async function accountFromAccessToken(token: string) {
  ...
  if (current.error || !current.data.user) throw new IdentityError(401, 'FaÃ§a login para continuar.');
