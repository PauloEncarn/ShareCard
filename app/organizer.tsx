...
import { Download as ArrowDownToLine, NavArrowRight as ArrowRight, Check, DoubleCheck as CheckCheck, NavArrowRight as ChevronRight, Cloud, CreditCard, Page as FileText, Dashboard as LayoutDashboard, Lock as LockKeyhole, Plus, Search, Settings as Settings2, ShieldCheck, Spark as Sparkles, Trash as Trash2, StatsUpSquare as TrendingUp, Upload, Group as Users, Wallet, Xmark as X, WarningCircle as CircleAlert, EditPencil as Pencil, Calendar as CalendarDays } from 'iconoir-react';
...
export default function Organizer({ initialTab = 'overview', personId }: { initialTab?: string; personId?: string }) {
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      setCandidateFile(file); setCandidate(guideBuyers(carryAssignments(parsed, state.statements.filter(s => s.id !== 'demo'))));
  ...
  function downloadBackup() {
    download(JSON.stringify(state, null, 2), 'fatura-backup.json', 'application/json');
  }
  function exportCsv() {
    if (!active) return;
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <input ref={backupInput} type="file" hidden accept=".json" onChange={async e => {
        ...
        try { if (file.size > 10 * 1024 * 1024) throw new Error('Backup muito grande.'); const restored = validateBackup(JSON.parse(await file.text())); if (state.statements.length) { downloadBackup(); } setStorageError(''); setState(restored); setMessage('Backup restaurado. Os dados anteriores foram exportados, se existiam.'); }
    ...
    <main>
      <header className="topbar"><div className="breadcrumb">Meu espaço <ChevronRight width={14} height={14}/><span>{({ overview: 'Visão geral', transactions: 'Lançamentos', people: 'Pessoas', forecast: 'Próximas faturas' } as Record<string, string>)[tab]}</span></div><span className="private-label"><LockKeyhole width={14} height={14}/> Local e privado</span></header>
      <div className="page-content">
        {storageError && <div className="notice danger" role="alert"><CircleAlert width={18} height={18}/>{storageError}</div>}
        <div className="page-heading"><div><div className="eyebrow">MENOS CONTAS NA CABEÇA</div><h1>{tab === 'overview' ? 'Sua fatura, sem mistério.' : tab === 'transactions' ? 'Cada compra no seu lugar.' : tab === 'people' ? 'Tudo dividido, tudo claro.' : 'Olhe os próximos meses.'}</h1><p>{tab === 'overview' ? 'Entenda os gastos, divida as compras e planeje o que vem.' : tab === 'transactions' ? 'Confira os lançamentos e escolha quem fica com cada valor.' : tab === 'people' ? 'Veja quanto cabe a cada pessoa nesta fatura.' : 'Acompanhe as parcelas que já estão comprometidas.'}</p></div><button className="button primary" disabled={busy || !ready} onClick={() => { setFormError(''); input.current?.click(); }}><Upload width={18} height={18}/>{busy ? 'Lendo fatura…' : 'Importar fatura'}</button><input ref={input} type="file" accept="application/pdf,.pdf" hidden onChange={e => upload(e.target.files?.[0])}/></div>

        {cloudMaster && <div className="statement-toolbar cloud-cycle-picker"><div className="statement-identity"><span className="bank-icon"><CreditCard width={18} height={18}/></span><div><label htmlFor="cloud-card">Cartão para a próxima importação</label><select id="cloud-card" value={cloudCardId} onChange={event => setCloudCardId(event.target.value)}><option value="">Selecione um cartão</option>{cloudCards.map(card => <option key={card.id} value={card.id}>{card.name} · vence dia {card.dueDay}</option>)}</select></div></div><span className="verified">O vencimento real virá do PDF</span></div>}
        {!active && tab !== 'people' ? <section className="empty-workspace" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); upload(e.dataTransfer.files[0]); }}><div className="empty-art"><FileText width={48} height={48}/><span><Check width={18} height={18}/></span></div><span className="pill">COMECE POR AQUI</span><h2>Uma fatura. Tudo organizado.</h2><p>Arraste sua fatura Itaú em PDF e descubra quanto é de cada pessoa e o que já está previsto para os próximos meses.</p><button className="button primary" onClick={() => input.current?.click()} disabled={busy || !cloudCardId}>{busy ? 'Lendo PDF…' : 'Selecionar minha fatura'}<ArrowRight width={18} height={18}/></button><button className="text-button" onClick={() => { setState(demoState()); setMessage('Exemplo carregado com dados fictícios.'); }}><Sparkles width={16} height={16}/> Explorar com um exemplo fictício</button><div className="empty-steps"><span><span>01</span> Importe o PDF</span><span><span>02</span> Divida as compras</span><span><span>03</span> Planeje o mês</span></div><small>PDF digital Itaú · Até 15 MB · Seu arquivo não é armazenado</small></section> : <>
          {active && <><div className="statement-toolbar"><div className="statement-identity"><span className="bank-icon">{active.id === 'demo' ? <CreditCard width={21} height={21}/> : 'itaú'}</span><div><label className="sr-only" htmlFor="statement">Fatura selecionada</label><select id="statement" value={active.id} onChange={e => { setState({ ...state, activeId: e.target.value }); setSearch(''); setFilter('all'); setCategory('all'); }}>{[...state.statements].sort((a, b) => b.dueDate.localeCompare(a.dueDate)).map(s => <option key={s.id} value={s.id}>{s.id === 'demo' ? 'Exemplo fictício' : 'Itaú'} · {monthLabel(s.dueDate)}</option>)}</select><span>Vencimento em {dateLabel(active.dueDate)}</span></div></div><div className="toolbar-actions"><span className="verified"><CheckCheck width={15} height={15}/>{sum(transactions) === active.total ? 'Total conferido' : 'Revisão necessária'}</span><button className="button secondary compact" onClick={exportCsv}><ArrowDownToLine width={15} height={15}/> Exportar CSV</button><button className="icon-button" aria-label="Excluir fatura" onClick={() => setDeleteId(active.id)}><Trash2 width={17} height={17}/></button></div></div>
          {active.id === 'demo' && <div className="demo-notice"><Sparkles width={15} height={15}/> Você está explorando dados fictícios. Importe seu PDF quando quiser.</div>}
        ...
        <div className="mobile-backup"><button className="text-button" onClick={downloadBackup}><ArrowDownToLine width={16} height={16}/> Exportar backup</button><button className="text-button" onClick={() => backupInput.current?.click()}><Upload width={16} height={16}/> Restaurar backup</button></div>
