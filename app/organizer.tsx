...
export default function Organizer({ initialTab = 'overview', personId }: { initialTab?: string; personId?: string }) {
  ...
  function openPerson(person?: Person) {
    setPersonEditing(person ?? null); setPersonName(person?.name ?? ''); setPersonEmail(person?.email ?? ''); setPersonLimit(person?.monthlyLimitCents ? (person.monthlyLimitCents / 100).toFixed(2).replace('.', ',') : ''); setPersonColor(person?.color ?? colors[state.people.length % colors.length]); setFormError(''); setPersonModal(true);
  }
  async function savePerson() {
    try {
      ...
      let person: Person = { id: personEditing?.id ?? crypto.randomUUID(), name, email: email || null, color: personColor, monthlyLimitCents, version: personEditing?.version };
      if (cloudMaster && (!personEditing || personEditing.version)) {
        const path = personEditing?.version ? `people/${personEditing.id}` : 'people';
        const response = await fetch(cloudEndpoint(path), { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email: email || null, color: personColor, monthlyLimitCents: monthlyLimitCents ?? null, ...(personEditing?.version ? { version: personEditing.version } : {}) }) });
        ...
        person = { id: saved.id, name: saved.name, email: saved.email ?? null, color: saved.color, monthlyLimitCents: saved.monthlyLimitCents ?? undefined, accountId: saved.accountId, version: saved.version };
      }
