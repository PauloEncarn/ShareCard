...
const cardLabel = (card: NonNullable<Statement['card']>) => `${card.issuer}${card.last4 ? ` Ã¢â¬Â¢Ã¢â¬Â¢Ã¢â¬Â¢Ã¢â¬Â¢ ${card.last4}` : ` ÃÂ· vence dia ${card.dueDay}`}`;
...
export default function Organizer({ initialTab = 'overview', personId }: { initialTab?: string; personId?: string }) {
  ...
  useEffect(() => {
    ...
    try {
      void Promise.resolve(context.registerTool({
        ...
        description: 'LÃÂª o total, os valores por pessoa e as parcelas previstas da fatura selecionada. NÃÂ£o altera dados.',
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      if (state.statements.some(s => s.fingerprint === parsed.fingerprint)) throw new Error('Esta fatura jÃÂ¡ foi importada. Selecione-a no histÃÂ³rico.');
    ...
    } catch (e) { setMessage(e instanceof Error ? e.message : 'NÃÂ£o foi possÃÂ­vel ler o PDF.'); }
  ...
  async function confirmImport() {
    ...
    if (state.statements.some(s => s.dueDate === candidate.dueDate && s.total === candidate.total && s.id !== 'demo')) { setFormError('JÃÂ¡ existe uma fatura com este vencimento e total. Confira o histÃÂ³rico antes de importar novamente.'); return; }
    if (cloudMaster) {
      ...
      try {
        ...
        if (!selectedCardId && candidate.card) {
          ...
          if (!response.ok) throw new Error(created.error || 'NÃÂ£o foi possÃÂ­vel cadastrar o cartÃÂ£o identificado.');
        ...
        if (!selectedCardId) throw new Error('NÃÂ£o identificamos o cartÃÂ£o. Selecione um cartÃÂ£o antes de importar.');
        if (cloudBackend === 'supabase') {
          ...
          if (!response.ok) throw new Error(result.error || 'NÃÂ£o foi possÃÂ­vel salvar a fatura na nuvem.');
        } else {
          ...
          const result = await response.json(); if (!response.ok) throw new Error(result.error || 'NÃÂ£o foi possÃÂ­vel salvar a fatura na nuvem.');
        }
        window.sessionStorage.setItem('sharecard:workspace-stale', '1');
      } catch (error) { setFormError(error instanceof Error ? error.message : 'NÃÂ£o foi possÃÂ­vel salvar a fatura na nuvem.'); setBusy(false); return; }
      setBusy(false);
    }
    ...
    setCandidate(null); setCandidateFile(null); setTab('overview'); setSearch(''); setFilter('all'); setCategory('all'); setFormError(''); setMessage(cloudMaster ? 'Fatura salva. Agora vocÃÂª pode dividir as compras.' : 'Fatura importada. Agora vocÃÂª pode dividir as compras.');
  }
  ...
  function savePerson() {
    try {
      ...
      if (!name || state.people.some(p => p.id !== personEditing?.id && p.name.toLocaleLowerCase() === name.toLocaleLowerCase())) throw new Error('Informe um nome diferente dos jÃÂ¡ cadastrados.');
  ...
  function openSplit(t: Transaction) {
    if (isSharedCost(t)) { setMessage('Este gasto geral ÃÂ© rateado automaticamente entre os compradores. Altere o tipo na ediÃÂ§ÃÂ£o para dividir individualmente.'); return; }
  ...
  function saveSplit() {
    ...
    try {
      ...
      if (!validateAllocations(split, allocations)) throw new Error('A divisÃÂ£o nÃÂ£o pode ultrapassar o valor da compra e deve manter o sinal do lanÃÂ§amento.');
      updateStatement(s => ({ ...s, transactions: s.transactions.map(t => t.id === split.id ? { ...t, allocations, carryForward: carry } : t) })); setSplit(null); setMessage('DivisÃÂ£o salva.');
    } catch (e) { setFormError((e as Error).message); }
  }
  function exportCsv() {
    if (!active) return;
    const cell = (s: string) => '"' + (/^[=+@-]/.test(s) ? "'" : '') + s.replaceAll('"', '""') + '"';
    const rows = [['Data', 'Estabelecimento', 'Categoria', 'Portador', 'Parcela', 'Valor', 'Comprador', 'ObservaÃÂ§ÃÂ£o', 'Tipo de rateio', 'DivisÃÂ£o', 'Sem responsÃÂ¡vel'], ...transactions.map(t => [t.date, t.merchant, t.category, t.holder, t.installment ? `${t.installment.current}/${t.installment.total}` : '', (t.cents / 100).toFixed(2).replace('.', ','), state.people.find(p => p.id === buyerOf(t))?.name ?? '', t.note ?? '', isSharedCost(t) ? 'Geral: rateio automÃÂ¡tico' : 'Individual', t.allocations.map(a => `${state.people.find(p => p.id === a.personId)?.name ?? 'Pessoa'}: ${money(a.cents)}`).join(' | '), (unassigned(t) / 100).toFixed(2).replace('.', ',')])];
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <a className="brand brand-logo" href="/organizador" aria-label="VisÃÂ£o geral do ShareCard"><img src="/brand/sharecard_symbol.png" alt="ShareCard"/></a>
      <div className="workspace-label">SEU ORGANIZADOR</div>
      <nav aria-label="NavegaÃÂ§ÃÂ£o principal">{[{ id: 'overview', label: 'VisÃÂ£o geral', icon: LayoutDashboard }, { id: 'transactions', label: 'LanÃÂ§amentos', icon: CreditCard }, { id: 'people', label: 'Pessoas', icon: Users }, { id: 'cards', label: 'CartÃÂµes e grupo', icon: CreditCard }, { id: 'forecast', label: 'PrÃÂ³ximas faturas', icon: TrendingUp }, { id: 'account', label: 'Minha conta', icon: Wallet }].map(item => <button key={item.id} className={`nav-item ${tab === item.id ? 'active' : ''}`} onClick={() => setTab(item.id)}><span className="nav-icon"><item.icon width={20} height={20}/></span><span>{item.label}</span>{tab === item.id && <span className="nav-mark"/>}</button>)}</nav>
      <div className="sidebar-note"><ShieldCheck width={24} height={24}/><strong>Seu dinheiro.<br/>Sua privacidade.</strong><p>Suas faturas e informaÃÂ§ÃÂµes ficam protegidas no seu espaÃÂ§o.</p><span><LockKeyhole width={12} height={12}/> Dados protegidos</span></div>
      <div className="local-user"><span className="user-avatar">EU</span><div>Meu espaÃÂ§o<small>Organize suas faturas</small></div></div>
    </aside>

    <main>
      <header className="topbar"><div className="breadcrumb">Meu espaÃÂ§o <ChevronRight width={14} height={14}/><span>{({ overview: 'VisÃÂ£o geral', transactions: 'LanÃÂ§amentos', people: 'Pessoas', forecast: 'PrÃÂ³ximas faturas', cards: 'CartÃÂµes e grupo', account: 'Minha conta' } as Record<string, string>)[tab]}</span></div><span className="private-label"><LockKeyhole width={14} height={14}/> EspaÃÂ§o protegido</span></header>
      <div className="page-content">
        {storageError && <div className="notice danger" role="alert"><CircleAlert width={18} height={18}/>{storageError}</div>}
        <div className="page-heading"><div><div className="eyebrow">MENOS CONTAS NA CABEÃâ¡A</div><h1>{tab === 'overview' ? 'Sua fatura, sem mistÃÂ©rio.' : tab === 'transactions' ? 'Cada compra no seu lugar.' : tab === 'people' ? 'Tudo dividido, tudo claro.' : tab === 'cards' ? 'CartÃÂµes e compradores.' : tab === 'account' ? 'Minha conta.' : 'Olhe os prÃÂ³ximos meses.'}</h1><p>{tab === 'overview' ? 'Entenda os gastos, divida as compras e planeje o que vem.' : tab === 'transactions' ? 'Confira os lanÃÂ§amentos e escolha quem fica com cada valor.' : tab === 'people' ? 'Veja quanto cabe a cada pessoa nesta fatura.' : tab === 'cards' ? 'Cadastre os ciclos, convide compradores e acompanhe as faturas do grupo.' : tab === 'account' ? 'Confira seu nome, e-mail, papel no grupo e foto de perfil.' : 'Acompanhe as parcelas que jÃÂ¡ estÃÂ£o comprometidas.'}</p></div>{tab !== 'account' && tab !== 'cards' && <button className="button primary" disabled={busy || !ready} onClick={() => { setFormError(''); input.current?.click(); }}><Upload width={18} height={18}/>{busy ? 'Lendo faturaÃ¢â¬Â¦' : 'Importar fatura'}</button>}<input ref={input} type="file" accept="application/pdf,.pdf" hidden onChange={e => upload(e.target.files?.[0])}/></div>

        {cloudMaster && tab !== 'account' && tab !== 'cards' && <div className="statement-toolbar cloud-cycle-picker"><div className="statement-identity"><span className="bank-icon"><CreditCard width={18} height={18}/></span><div><label htmlFor="cloud-card">CartÃÂ£o para a prÃÂ³xima importaÃÂ§ÃÂ£o</label><select id="cloud-card" value={cloudCardId} onChange={event => setCloudCardId(event.target.value)}><option value="">Selecione um cartÃÂ£o</option>{cloudCards.map(card => <option key={card.id} value={card.id}>{card.name} ÃÂ· vence dia {card.dueDay}</option>)}</select></div></div><span className="verified">O vencimento real virÃÂ¡ do PDF</span></div>}
        {cloudBuyer && tab === 'overview' ? <BuyerPanel summary={buyerSummary}/> : tab === 'cards' ? <CloudWorkspace embedded/> : tab === 'account' ? <CloudWorkspace embedded section="profile"/> : !active && tab !== 'people' ? <section className="empty-workspace" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); upload(e.dataTransfer.files[0]); }}><div className="empty-art"><FileText width={48} height={48}/><span><Check width={18} height={18}/></span></div><span className="pill">COMECE POR AQUI</span><h2>Uma fatura. Tudo organizado.</h2><p>Arraste sua fatura ItaÃÂº em PDF e descubra quanto ÃÂ© de cada pessoa e o que jÃÂ¡ estÃÂ¡ previsto para os prÃÂ³ximos meses.</p><button className="button primary" onClick={() => input.current?.click()} disabled={busy || !cloudCardId}>{busy ? 'Lendo PDFÃ¢â¬Â¦' : 'Selecionar minha fatura'}<ArrowRight width={18} height={18}/></button><div className="empty-steps"><span><span>01</span> Importe o PDF</span><span><span>02</span> Divida as compras</span><span><span>03</span> Planeje o mÃÂªs</span></div><small>PDF digital ItaÃÂº ÃÂ· AtÃÂ© 15 MB ÃÂ· Arquivo protegido no seu espaÃÂ§o</small></section> : <>
          {active && <><div className="statement-toolbar"><div className="statement-identity"><span className="bank-icon"><CreditCard width={21} height={21}/></span><div><label className="sr-only" htmlFor="statement">Fatura selecionada</label><select id="statement" value={active.id} onChange={e => { setState({ ...state, activeId: e.target.value }); setSearch(''); setFilter('all'); setCategory('all'); }}>{[...state.statements].sort((a, b) => b.dueDate.localeCompare(a.dueDate)).map(s => <option key={s.id} value={s.id}>ItaÃÂº ÃÂ· {monthLabel(s.dueDate)}</option>)}</select><span>Vencimento em {dateLabel(active.dueDate)}</span></div></div><div className="toolbar-actions"><span className="verified"><CheckCheck width={15} height={15}/>{sum(transactions) === active.total ? 'Total conferido' : 'RevisÃÂ£o necessÃÂ¡ria'}</span><button className="button secondary compact" onClick={exportCsv}><ArrowDownToLine width={15} height={15}/> Exportar CSV</button><button className="icon-button" aria-label="Excluir fatura" onClick={() => setDeleteId(active.id)}><Trash2 width={17} height={17}/></button></div></div>
          {sum(transactions) !== active.total && <div className="notice danger" role="alert"><CircleAlert width={17} height={17}/>Os lanÃÂ§amentos diferem do total do banco em {money(active.total - sum(transactions))}. Confira as correÃÂ§ÃÂµes antes de usar os valores.</div>}
          </>}
          ...
          {active && <>
          {(tab === 'overview' || tab === 'transactions') && <div className="stats-grid"><article className="stat-card main-stat"><div className="stat-label">Total da fatura <CreditCard width={19} height={19}/></div><strong>{money(active.total)}</strong><div className="stat-foot">{transactions.length} lanÃÂ§amentos <span>Vence {dateLabel(active.dueDate).slice(0, 5)}</span></div></article><article className="stat-card"><div className="stat-label">JÃÂ¡ distribuÃÂ­do <Users width={19} height={19}/></div><strong>{money(sum(transactions) - pending)}</strong><div className="progress-track"><span style={{ width: `${transactions.length ? allocatedCount / transactions.length * 100 : 0}%` }}/></div><small>{allocatedCount} de {transactions.length} lanÃÂ§amentos completos</small></article><article className="stat-card"><div className="stat-label">Falta dividir <span className="amber-icon"><Settings2 width={19} height={19}/></span></div><strong>{money(pending)}</strong><button className="stat-link" onClick={() => { setTab('transactions'); setFilter('pending'); }}>{pendingCount ? `${pendingCount} lanÃÂ§amentos para revisar` : 'Tudo atribuÃÂ­do'}<ArrowRight width={15} height={15}/></button></article></div>}

          {(tab === 'overview' || tab === 'forecast') && <section className="panel forecast-panel"><div className="panel-heading"><div><h2>O que jÃÂ¡ vem por aÃÂ­ <span className="soft-tag">PARCELAS</span></h2><p>Compromissos existentes, sem novas compras.</p></div>{tab === 'overview' ? <button className="text-button" onClick={() => setTab('forecast')}>Ver projeÃÂ§ÃÂ£o <ArrowRight width={16} height={16}/></button> : <select aria-label="PrevisÃÂ£o por pessoa" value={forecastPerson} onChange={e => setForecastPerson(e.target.value)}><option value="all">Todas as pessoas</option>{state.people.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>}</div><div className="forecast-content"><div className="forecast-summary"><span>PrÃÂ³ximo mÃÂªs</span><strong>{money(projections[0]?.cents ?? 0)}</strong><p>{forecastPerson === 'all' && active.nextTotal !== undefined ? `Banco informa ${money(active.nextTotal)} em parcelas.` : 'Conforme a divisÃÂ£o das parcelas futuras.'}</p><span className="forecast-caption"><CalendarDays width={15} height={15}/> Valores sujeitos a ajustes</span></div><div className="chart" role="img" aria-label={chart.map(p => `${p.label}: ${money(p.cents)}`).join('; ')}>{chart.map((p, i) => <div className="chart-column" key={p.label}><span className="chart-value">{money(p.cents)}</span><div className="bar-area"><div className={`bar ${i === 0 ? 'first' : ''}`} style={{ height: `${Math.max(3, p.cents / maxChart * 100)}%` }}/></div><span className="chart-label">{p.label}</span></div>)}</div></div>{tab === 'forecast' && <><div className="notice"><CircleAlert width={17} height={17}/><span>A prÃÂ³xima parcela usa o valor do PDF quando identificado. Os meses seguintes repetem esse valor como estimativa. Assinaturas, novas compras, juros e tarifas futuras nÃÂ£o estÃÂ£o incluÃÂ­dos.</span></div><div className="projection-list">{projections.map((p, i) => <div key={p.label}><span>{p.label}</span><strong>{money(p.cents)}</strong><small>{i === 0 ? 'Parcelas identificadas ou estimadas' : 'Estimativa das parcelas restantes'}</small></div>)}</div></>}</section>}
          ...
          {tab === 'overview' && <section className="people-section"><div className="section-heading"><div><h2>Quem fica com quanto</h2></div><button className="text-button" onClick={() => openPerson()}><Plus width={17} height={17}/> Adicionar pessoa</button></div><div className="people-grid">{state.people.map(p => {
            ...
            return <button className="person-card" key={p.id} onClick={() => router.push('/pessoas/' + encodeURIComponent(p.id))}><div><Avatar name={p.name} color={p.color}/><span><strong>{p.name}</strong><small>{count} lanÃÂ§amentos</small></span><ChevronRight width={17} height={17}/></div><strong className="person-total">{money(amount)}</strong><div className="person-line" style={{ background: p.color + '18' }}><span style={{ width: `${Math.min(100, Math.max(0, amount / Math.max(active.total, 1) * 100))}%`, background: p.color }}/></div></button>;
          })}<button className="add-person-card" onClick={() => openPerson()}><span><Plus width={22} height={22}/></span><strong>Mais alguÃÂ©m na conta?</strong><small>Adicione uma pessoa para dividir</small></button></div></section>}

          {(tab === 'overview' || tab === 'transactions') && <ChargeSummary statement={active} people={state.people}/>}
          {(tab === 'overview' || tab === 'transactions') && <section className="panel transaction-panel"><div className="panel-heading"><div><h2>LanÃÂ§amentos <span className="count-tag">{transactions.length}</span></h2><p>Atribua uma pessoa ou divida uma compra.</p></div>{tab === 'overview' && <button className="text-button" onClick={() => setTab('transactions')}>Ver todos <ArrowRight width={16} height={16}/></button>}</div><div className="filters"><label className="search-box"><Search width={18} height={18}/><input placeholder="Buscar compra, observaÃÂ§ÃÂ£o ou pessoa" value={search} onChange={e => setSearch(e.target.value)}/></label><select aria-label="Filtrar por responsÃÂ¡vel" value={filter} onChange={e => setFilter(e.target.value)}><option value="all">Todos os responsÃÂ¡veis</option><option value="pending">Falta dividir</option>{state.people.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><select aria-label="Filtrar por categoria" value={category} onChange={e => setCategory(e.target.value)}><option value="all">Todas as categorias</option>{[...new Set(transactions.map(t => t.category))].sort().map(c => <option key={c}>{c}</option>)}</select></div><div className="table-scroll"><table><thead><tr><th>ESTABELECIMENTO</th><th>DATA</th><th>PARCELA</th><th>VALOR</th><th>COMPRADOR / DIVISÃÆO</th><th><span className="sr-only">AÃÂ§ÃÂµes</span></th></tr></thead><tbody>{(tab === 'overview' ? visible.slice(0, 7) : visible).map(t => <tr key={t.id}><td><div className="merchant-cell"><span className={`merchant-icon ${t.kind === 'service' ? 'service' : ''}`}><CreditCard width={17} height={17}/></span><div><strong>{t.merchant}</strong><small>{t.category} ÃÂ· {t.holder}</small>{t.note && <span className="purchase-note">{t.note}</span>}</div></div></td><td className="muted">{t.date}</td><td>{t.installment ? <span className="installment-tag">{t.installment.current} de {t.installment.total}</span> : <span className="muted">Ãâ¬ vista</span>}</td><td className="amount-cell">{money(t.cents)}</td><td>{isSharedCost(t) ? <span className="shared-label">{t.allocations.length ? `Rateado entre ${t.allocations.length}` : 'Aguardando compradores'}</span> : <><span className="purchase-control-label">Quem comprou</span><select className="buyer-select" aria-label={`Comprador de ${t.merchant}`} value={buyerOf(t) ?? ''} onChange={e => setBuyer(t.id, e.target.value)}><option value="">Indicar comprador</option>{state.people.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><span className="purchase-control-label">Quem paga</span><div className="assign-cell"><select aria-label={`ResponsÃÂ¡vel por ${t.merchant}, ${t.date}, ${money(t.cents)}`} value={t.allocations.length === 1 && sum(t.allocations) === t.cents ? t.allocations[0].personId : t.allocations.length ? '__split' : ''} onChange={e => { if (e.target.value === '__split') openSplit(t); else assign(t.id, e.target.value); }} className={!t.allocations.length ? 'unassigned-select' : ''}><option value="">Definir pessoa</option>{t.allocations.length > 0 && <option value="__split">{t.allocations.length > 1 ? `Dividido entre ${t.allocations.length}` : 'DivisÃÂ£o parcial'}</option>}{state.people.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><button className="icon-button" aria-label={`Dividir ${t.merchant}`} onClick={() => openSplit(t)}><Users width={17} height={17}/></button></div></>}</td><td><button className="icon-button" aria-label={`Editar ${t.merchant}`} onClick={() => { setEditing(t); setFormError(''); }}><Pencil width={15} height={15}/></button></td></tr>)}</tbody></table>{!visible.length && <div className="empty-table">Nenhum lanÃÂ§amento corresponde aos filtros.</div>}</div><div className="table-footer"><span>{tab === 'overview' ? Math.min(7, visible.length) : visible.length} de {transactions.length} lanÃÂ§amentos</span><span>Total filtrado <strong>{money(sum(visible))}</strong></span></div></section>}
          </>}
        </>}
        <footer className="page-footer"><span><ShieldCheck width={14} height={14}/> Seus dados ficam protegidos no seu espaÃÂ§o compartilhado.</span><span>Fatura em dia ÃÂ· versÃÂ£o inicial</span></footer>
      </div>
    </main>
    ...
    {personModal && <Modal title={personEditing ? 'Editar pessoa' : 'Adicionar pessoa'} onClose={() => setPersonModal(false)}><form onSubmit={e => { e.preventDefault(); savePerson(); }}><p className="modal-description">Uma pessoa tem suas compras, sua parte dos encargos e um limite opcional por fatura.</p><label className="field">Nome<input autoFocus maxLength={40} value={personName} onChange={e => setPersonName(e.target.value)} placeholder="Ex.: Maria" required/></label><label className="field">Limite por fatura (R$)<input inputMode="decimal" value={personLimit} onChange={e => setPersonLimit(e.target.value)} placeholder="Opcional. Ex.: 800,00"/></label><label className="field">Cor de identificaÃÂ§ÃÂ£o<input type="color" value={personColor} onChange={e => setPersonColor(e.target.value)}/></label>{formError && <p className="error-text" role="alert">{formError}</p>}<div className="modal-actions"><button type="button" className="button secondary" onClick={() => setPersonModal(false)}>Cancelar</button><button className="button primary">{personEditing ? 'Salvar pessoa' : 'Adicionar pessoa'}</button></div></form></Modal>}
    {personDelete && <Modal title="Excluir pessoa?" onClose={() => setPersonDelete(null)}><p>Excluir {personDelete.name} do cadastro? Pessoas com compras ou divisÃÂµes em qualquer fatura precisam ser desvinculadas antes.</p>{formError && <p className="error-text" role="alert">{formError}</p>}<div className="modal-actions"><button className="button secondary" onClick={() => setPersonDelete(null)}>Cancelar</button><button className="button danger-button" onClick={() => { const id = personDelete.id; if (state.statements.some(s => s.transactions.some(t => buyerOf(t) === id || t.allocations.some(a => a.personId === id)))) { setFormError('Esta pessoa tem compras ou divisÃÂµes vinculadas. Altere os vÃÂ­nculos antes de excluir.'); return; } setState(prev => ({ ...prev, people: prev.people.filter(p => p.id !== id) })); setPersonDelete(null); if (personId === id) router.push('/pessoas'); }}>Excluir pessoa</button></div></Modal>}
    {split && <Modal title="Dividir compra" onClose={() => setSplit(null)}><div className="split-summary"><div><strong>{split.merchant}</strong><small>{split.date}{split.installment ? ` ÃÂ· Parcela ${split.installment.current}/${split.installment.total}` : ''}</small></div><strong>{money(split.cents)}</strong></div>{!state.people.length ? <div className="notice">Cadastre uma pessoa na seÃÂ§ÃÂ£o Pessoas para comeÃÂ§ar a dividir.</div> : <><div className="section-heading"><p>Escolha quem participa</p><button className="text-button" onClick={() => equalSplit()}>Dividir igualmente</button></div><div className="split-people">{state.people.map(p => <div className="split-row" key={p.id}><label><input type="checkbox" checked={splitPeople.includes(p.id)} onChange={e => { const ids = e.target.checked ? [...splitPeople, p.id] : splitPeople.filter(id => id !== p.id); setSplitPeople(ids); equalSplit(ids); }}/><Avatar name={p.name} color={p.color} small/>{p.name}</label>{splitPeople.includes(p.id) && <label className="currency-input"><span>R$</span><input aria-label={`Valor de ${p.name}`} inputMode="decimal" value={splitValues[p.id] ?? ''} onChange={e => setSplitValues({ ...splitValues, [p.id]: e.target.value })}/></label>}</div>)}</div>{split.installment && <label className="check-field"><input type="checkbox" checked={carry} onChange={e => setCarry(e.target.checked)}/> Aplicar esta proporÃÂ§ÃÂ£o ÃÂ s prÃÂ³ximas parcelas reconhecidas</label>}<p className="helper">VocÃÂª pode deixar parte do valor sem responsÃÂ¡vel para decidir depois.</p></>}{formError && <p className="error-text" role="alert">{formError}</p>}<div className="modal-actions"><button className="button secondary" onClick={() => setSplit(null)}>Cancelar</button><button className="button primary" disabled={!state.people.length} onClick={saveSplit}>Salvar divisÃÂ£o</button></div></Modal>}
    {candidate && <Modal title="Confira sua fatura" onClose={() => { setCandidate(null); setFormError(''); }} wide><p className="modal-description">{candidate.filename} ÃÂ· {candidate.transactions.length} lanÃÂ§amentos encontrados. {candidate.card ? <>CartÃÂ£o identificado: <strong>{candidate.card.issuer}{candidate.card.last4 ? ` Ã¢â¬Â¢Ã¢â¬Â¢Ã¢â¬Â¢Ã¢â¬Â¢ ${candidate.card.last4}` : ''}</strong> ÃÂ· vence dia {candidate.card.dueDay}.</> : 'Confira os dados antes de salvar.'}</p><div className="review-summary"><div><small>Total da fatura</small><strong>{money(candidate.total)}</strong></div><div><small>Soma dos lanÃÂ§amentos</small><strong>{money(sum(candidate.transactions))}</strong></div><div><small>DiferenÃÂ§a</small><strong className={candidate.total === sum(candidate.transactions) ? 'success-text' : 'error-text'}>{money(candidate.total - sum(candidate.transactions))}</strong></div></div>{candidate.warnings.length > 0 && <details className="review-warnings"><summary>{candidate.warnings.length} observaÃÂ§ÃÂµes da leitura</summary><ul>{candidate.warnings.map((w, i) => <li key={i}>{w}</li>)}</ul></details>}<p className="helper">Os responsÃÂ¡veis ficam em branco. Compras parceladas reconhecidas de uma fatura anterior podem herdar a divisÃÂ£o.</p><div className="review-table table-scroll"><table><thead><tr><th>Compra / portador</th><th>Data</th><th>Parcela</th><th>Valor</th><th>Comprador</th><th>AÃÂ§ÃÂµes</th></tr></thead><tbody>{candidate.transactions.map(t => <tr key={t.id}><td><strong>{t.merchant}</strong><small className="block">{t.holder}</small></td><td>{t.date}</td><td>{t.installment ? `${t.installment.current}/${t.installment.total}` : 'Ã¢â¬â'}</td><td>{money(t.cents)}</td><td><select aria-label={`Comprador de ${t.merchant}`} value={buyerOf(t) ?? ""} disabled={isSharedCost(t)} onChange={event => setCandidateBuyer(t.id, event.target.value)}><option value="">Decidir depois</option><option value="__master__">Minha compra</option>{state.people.map(person => <option key={person.id} value={person.id}>{person.name}</option>)}</select></td><td><button className="icon-button" aria-label={`Corrigir ${t.merchant}`} onClick={() => { setEditing(t); setFormError(''); }}><Pencil width={16} height={16}/></button><button className="icon-button" aria-label={`Remover ${t.merchant}`} onClick={() => setCandidate({ ...candidate, transactions: candidate.transactions.filter(x => x.id !== t.id) })}><Trash2 width={16} height={16}/></button></td></tr>)}</tbody></table></div><div className="review-tools"><button className="text-button" onClick={() => { const t: Transaction = { id: crypto.randomUUID(), date: '01/08', merchant: 'Novo lanÃÂ§amento', cents: 0, category: 'Outros', holder: 'NÃÂ£o identificado', kind: 'purchase', allocations: [], carryForward: true }; setCandidate({ ...candidate, transactions: [...candidate.transactions, t] }); setEditing(t); setFormError(''); }}><Plus width={16} height={16}/> Adicionar lanÃÂ§amento ausente</button></div>{formError && <p className="error-text" role="alert">{formError}</p>}<div className="modal-actions"><span className="helper">{sum(candidate.transactions) === candidate.total ? 'Total conferido. Pronto para importar.' : 'Corrija a diferenÃÂ§a para continuar.'}</span><button className="button secondary" onClick={() => setCandidate(null)}>Cancelar</button><button className="button primary" disabled={sum(candidate.transactions) !== candidate.total || !candidate.transactions.length} onClick={confirmImport}><Check width={17} height={17}/> Confirmar importaÃÂ§ÃÂ£o</button></div></Modal>}
    {editing && <Modal title="Editar compra" onClose={() => setEditing(null)}><form onSubmit={e => { e.preventDefault(); editSave(new FormData(e.currentTarget)); }}><label className="field">Estabelecimento<input name="merchant" defaultValue={editing.merchant} required maxLength={150}/></label><div className="two-fields"><label className="field">Data (DD/MM)<input name="date" defaultValue={editing.date} required pattern="[0-9]{2}/[0-9]{2}"/></label><label className="field">Valor (R$)<input name="amount" defaultValue={(editing.cents / 100).toFixed(2).replace('.', ',')} inputMode="decimal" required/></label></div><label className="field">Comprador<select name="buyerId" defaultValue={buyerOf(editing) ?? ''}><option value="">Ainda nÃÂ£o identificado</option>{state.people.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label className="field">ObservaÃÂ§ÃÂ£o<textarea name="note" defaultValue={editing.note ?? ''} maxLength={2000} rows={3} placeholder="Ex.: presente de aniversÃÂ¡rio, compra para a casaÃ¢â¬Â¦"/></label><label className="check-field"><input type="checkbox" name="sharedCost" defaultChecked={isSharedCost(editing)}/> Gasto geral: dividir igualmente entre todos os compradores</label><p className="helper">Use para encargos, juros, IOF, seguros e outras despesas comuns. Neste caso o comprador individual ÃÂ© ignorado.</p><label className="field">Categoria<input name="category" defaultValue={editing.category} maxLength={40}/></label><div className="two-fields"><label className="field">Parcela atual<input type="number" name="current" min={1} max={60} defaultValue={editing.installment?.current}/></label><label className="field">Total de parcelas<input type="number" name="total" min={1} max={60} defaultValue={editing.installment?.total}/></label></div><p className="helper">Deixe as parcelas vazias para compras ÃÂ  vista. Ao alterar o valor, a divisÃÂ£o existente serÃÂ¡ ajustada proporcionalmente.</p>{formError && <p className="error-text" role="alert">{formError}</p>}<div className="modal-actions"><button type="button" className="button secondary" onClick={() => setEditing(null)}>Cancelar</button><button className="button primary">Salvar correÃÂ§ÃÂ£o</button></div></form></Modal>}
    {deleteId && <Modal title="Excluir esta fatura?" onClose={() => setDeleteId(null)}><p className="modal-description">Os lanÃÂ§amentos e suas divisÃÂµes serÃÂ£o removidos deste navegador. As outras faturas e pessoas serÃÂ£o mantidas.</p><div className="modal-actions"><button className="button secondary" onClick={() => setDeleteId(null)}>Cancelar</button><button className="button danger-button" onClick={() => { setState(prev => ({ ...prev, statements: prev.statements.filter(s => s.id !== deleteId), activeId: prev.statements.find(s => s.id !== deleteId)?.id ?? null })); setDeleteId(null); }}>Excluir fatura</button></div></Modal>}
  </div>;
}
