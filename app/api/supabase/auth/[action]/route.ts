...
async function input(request: Request) {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new IdentityError(400, 'Dados inválidos.');
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    if (!result) throw new IdentityError(404, 'Rota não encontrada.');
    ...
    return Response.json({ error: error instanceof IdentityError ? error.message : 'Serviço de identidade indisponível.' }, { status: error instanceof IdentityError ? error.status : 503, headers });
  }
}

export async function GET(request: Request, context: Context) {
  try {
    if ((await context.params).action !== 'me') throw new IdentityError(404, 'Rota não encontrada.');
...
export async function DELETE(_request: Request, context: Context) {
  try {
    if ((await context.params).action !== 'logout') throw new IdentityError(404, 'Rota não encontrada.');
    const response = Response.json({ ok: true }, { headers });
    response.headers.append('Set-Cookie', cookie('', 0));
    return response;
  } catch (error) {
