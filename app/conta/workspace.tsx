...
export default function CloudWorkspace({ embedded = false }: { embedded?: boolean }) {
  ...
  return <main className={`cloud-shell ${embedded ? "account-embedded" : ""}`}><header className="cloud-top"><a href="/organizador" className="cloud-brand"><img src="/brand/sharecard_logo.png" alt="ShareCard"/></a><nav className="cloud-nav" aria-label="Navegação principal"><a href="/organizador">Organizador</a><a href="/pessoas">Pessoas</a><a className="active" href="/conta">Conta</a></nav><button className="profile-photo-button" type="button" onClick={() => avatarInput.current?.click()} aria-label="Alterar sua foto de perfil"><ProfilePhoto account={account}/><EditPencil/></button><div className="cloud-user"><span>{account.name}</span><small>{account.role === 'master' ? 'Master' : 'Comprador'}</small></div><button className="cloud-logout" onClick={logout}><LogOut/> <span>Sair</span></button></header><section className="cloud-content"><input ref={avatarInput} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => uploadAvatar(event.target.files?.[0])}/><div className="cloud-title"><div><span className="cloud-kicker">ESPAÇO COMPARTILHADO</span><h1>{account.role === 'master' ? 'Seu cofre de faturas.' : 'Faturas do seu grupo.'}</h1><p>{account.role === 'master' ? 'Cadastre seus ciclos, guarde PDFs e mantenha os compradores no mesmo contexto.' : 'Consulte os cartões e baixe os documentos compartilhados pelo master.'}</p></div><div className="cloud-status"><Check/><span>Espaço ativo<small>{documents.length} {documents.length === 1 ? 'fatura salva' : 'faturas salvas'}</small></span></div></div><button className="avatar-callout" type="button" onClick={() => avatarInput.current?.click()} disabled={busy}><ProfilePhoto account={account}/><span><strong>Sua foto no grupo</strong><small>JPEG, PNG ou WebP · até 2 MB · guardada no bucket privado</small></span><EditPencil/></button>{error && <p className="cloud-error" role="alert">{error}</p>}{notice && <p className="cloud-notice" role="status">{notice}<button onClick={() => setNotice('')}>Fechar</button></p>}
