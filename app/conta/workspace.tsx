...
export default function CloudWorkspace({ embedded = false, section = 'management' }: { embedded?: boolean; section?: 'profile' | 'management' }) {
  ...
  <section className="cycle-section"><div className="section-line"><div><span>SEUS CICLOS</span><h2>Cartões e vencimentos</h2></div><strong>{cards.length}</strong></div>{cards.length ? <div className="cycle-list">{cards.map(card => <article key={card.id}><span className="due-day">{String(card.dueDay).padStart(2,'0')}</span><div><strong>{card.name}</strong><small>Vencimento mensal no dia {card.dueDay}</small></div>{account.role === 'master' && <button className="text-button" type="button" disabled={busy} onClick={() => removeCard(card)}>Excluir</button>}</article>)}</div> : <p className="cloud-empty">{account.role === 'master' ? 'Cadastre o primeiro cartão para liberar o envio de PDFs.' : 'O master ainda não cadastrou cartões.'}</p>}</section>
