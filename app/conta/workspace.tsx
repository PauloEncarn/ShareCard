...
const api = async <T,>(path: string, options?: RequestInit): Promise<T> => {
  ...
  if (!response.ok) throw new Error(data?.error || 'NÃƒÆ’Ã‚Â£o foi possÃƒÆ’Ã‚Â­vel concluir esta aÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.');
  return data as T;
};
...
export default function CloudWorkspace() {
  ...
  async function createCard(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setError(''); const form = new FormData(event.currentTarget);
    try { await api('cards', json({ name: form.get('name'), dueDay: Number(form.get('dueDay')) })); await refresh(account!); event.currentTarget.reset(); setNotice('CartÃƒÆ’Ã‚Â£o cadastrado.'); } catch (reason) { setError((reason as Error).message); } finally { setBusy(false); }
  }
  async function createInvite(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setError(''); const form = new FormData(event.currentTarget);
    try { const result = await api<{token:string}>('invites', json({ email: form.get('email') })); setInvite(`${window.location.origin}/conta?invite=${result.token}`); setNotice('Link de convite criado. Copie e envie ÃƒÆ’Ã‚Â  pessoa.'); event.currentTarget.reset(); } catch (reason) { setError((reason as Error).message); } finally { setBusy(false); }
  }
  async function upload(file?: File, cardId?: string, dueDate?: string) {
    if (!file || !cardId || !dueDate) return; setBusy(true); setError('');
    try { await api(`documents?cardId=${encodeURIComponent(cardId)}&dueDate=${dueDate}`, { method: 'POST', body: file, headers: { 'Content-Type': 'application/pdf' } }); await refresh(account!); setNotice('PDF salvo no espaÃƒÆ’Ã‚Â§o privado do master.'); } catch (reason) { setError((reason as Error).message); } finally { setBusy(false); if (fileInput.current) fileInput.current.value = ''; }
  }
  async function uploadAvatar(file?: File) {
    if (!file) return;
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type) || file.size > 2 * 1024 * 1024) { setError('Use uma foto JPEG, PNG ou WebP de atÃƒÆ’Ã‚Â© 2 MB.'); return; }
    setBusy(true); setError('');
    try { const updated = await api<Account>('avatar', { method: 'POST', body: file, headers: { 'Content-Type': file.type } }); setAccount(updated); await refresh(updated); setNotice('Foto de perfil salva no espaÃƒÆ’Ã‚Â§o privado do grupo.'); }
  ...
  if (loading) return <main className="cloud-loading"><Cloud/><span>Conectando ao seu espaÃƒÆ’Ã‚Â§oÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦</span></main>;
  if (!account) return <main className="auth-shell auth-experience">
    ...
    <section className="auth-story">
      ...
      <div className="auth-illustration" aria-hidden="true"><div className="auth-mini-card auth-mini-total"><Wallet/><span><strong>R$ 320,00</strong><small>Dividido entre 4 pessoas</small></span><Check/></div><div className="auth-flow"><div className="auth-flow-card auth-flow-people"><Users/><span>Amigos<br/>e famÃƒÂ­lia</span></div><div className="auth-flow-card auth-flow-shopping"><CreditCard/><span>Compras<br/>compartilhadas</span></div><div className="auth-flow-mark"><img src="/brand/sharecard_symbol.png" alt=""/></div><div className="auth-flow-card auth-flow-split"><FileText/><span>DivisÃƒÂ£o<br/>automÃƒÂ¡tica</span></div><div className="auth-flow-card auth-flow-paid"><Check/><span>Tudo<br/>organizado</span></div></div><div className="auth-mini-card auth-mini-paid"><FileText/><span>Pagamento<br/><strong>confirmado!</strong></span><Check/></div></div>
      <ul className="auth-benefits"><li><ShieldCheck/> Seguro e confiÃƒÂ¡vel</li><li><Users/> Feito para compartilhar</li><li><CalendarDays/> Mais controle no dia a dia</li></ul>
    </section>
    <section className="auth-panel-wrap"><p className="auth-aside-note">Pagamentos mais simples<br/>para grandes momentos.</p><a className="back-link" href="/"><ArrowLeft/> Voltar</a><div className="auth-form">
      ...
      <form onSubmit={authenticate}>{mode === 'register' && <label>Seu nome<input name="name" required maxLength={100} autoComplete="name" placeholder="Seu nome completo"/></label>}<label>E-mail<input name="email" type="email" required autoComplete="email" placeholder="seu@email.com"/></label><label>Senha<input name="password" type="password" minLength={6} maxLength={128} required autoComplete={mode === 'login' ? 'current-password' : 'new-password'} placeholder={mode === 'login' ? 'Digite sua senha' : 'Crie uma senha segura'}/><small>Use pelo menos 6 caracteres.</small></label>{mode === 'register' && <label>CÃƒÂ³digo de convite <span>opcional</span><input name="inviteToken" maxLength={64} placeholder="Cole seu cÃƒÂ³digo"/><small>Sem código, você cria um espaço master.</small></label>}{error && <p className="cloud-error" role="alert">{error}</p>}<button className="button primary auth-submit" disabled={busy}>{busy ? 'AguardeÃ¢â‚¬Â¦' : mode === 'login' ? <>Entrar <ArrowRight/></> : <>Criar conta <ArrowRight/></>}</button></form>
      <p className="auth-switch">{mode === 'login' ? <>Ainda nÃƒÂ£o tem uma conta? <button type="button" onClick={() => { setMode('register'); setError(''); }}>Criar conta</button></> : <>JÃƒÂ¡ possui uma conta? <button type="button" onClick={() => { setMode('login'); setError(''); }}>Entrar</button></>}</p><p className="local-mode"><ShieldCheck/> Seus dados sÃƒÂ£o protegidos e seguros.</p>
    </div></section>
  </main>;
  const cardName = (id: string) => cards.find(card => card.id === id)?.name || 'CartÃƒÆ’Ã‚Â£o';
  return <main className="cloud-shell"><header className="cloud-top"><a href="/" className="cloud-brand"><img src="/brand/sharecard_logo.png" alt="ShareCard"/></a><button className="profile-photo-button" type="button" onClick={() => avatarInput.current?.click()} aria-label="Alterar sua foto de perfil"><ProfilePhoto account={account}/><EditPencil/></button><div><span>{account.name}</span><small>{account.role === 'master' ? 'Master Ãƒâ€šÃ‚Â· dono do cartÃƒÆ’Ã‚Â£o' : 'Comprador vinculado'}</small></div><button onClick={logout}><LogOut/> Sair</button></header><section className="cloud-content"><input ref={avatarInput} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => uploadAvatar(event.target.files?.[0])}/><div className="cloud-title"><div><span className="cloud-kicker">ESPAÃƒÆ’Ã¢â‚¬Â¡O COMPARTILHADO</span><h1>{account.role === 'master' ? 'Seu cofre de faturas.' : 'Faturas do seu grupo.'}</h1><p>{account.role === 'master' ? 'Cadastre seus ciclos, guarde PDFs e mantenha os compradores no mesmo contexto.' : 'Consulte os cartÃƒÆ’Ã‚Âµes e baixe os documentos compartilhados pelo master.'}</p></div><div className="cloud-status"><Check/><span>EspaÃƒÆ’Ã‚Â§o ativo<small>{documents.length} {documents.length === 1 ? 'fatura salva' : 'faturas salvas'}</small></span></div></div><button className="avatar-callout" type="button" onClick={() => avatarInput.current?.click()} disabled={busy}><ProfilePhoto account={account}/><span><strong>Sua foto no grupo</strong><small>JPEG, PNG ou WebP Ãƒâ€šÃ‚Â· atÃƒÆ’Ã‚Â© 2 MB Ãƒâ€šÃ‚Â· guardada no bucket privado</small></span><EditPencil/></button>{error && <p className="cloud-error" role="alert">{error}</p>}{notice && <p className="cloud-notice" role="status">{notice}<button onClick={() => setNotice('')}>Fechar</button></p>}
  {account.role === 'master' && <div className="master-tools"><section><div className="tool-title"><CreditCard/><div><h2>Novo cartÃƒÆ’Ã‚Â£o</h2><p>Use um cadastro para cada ciclo.</p></div></div><form onSubmit={createCard}><label>Apelido<input name="name" required placeholder="Ex.: ItaÃƒÆ’Ã‚Âº principal" maxLength={100}/></label><label>Vence todo dia<input name="dueDay" type="number" min="1" max="31" required placeholder="06"/></label><button disabled={busy}><Plus/> Adicionar</button></form></section><section><div className="tool-title"><UserPlus/><div><h2>Convidar comprador</h2><p>O cÃƒÆ’Ã‚Â³digo vale por 48 horas e uma utilizaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.</p></div></div><form onSubmit={createInvite}><label>E-mail<input name="email" type="email" required placeholder="pessoa@email.com"/></label><button disabled={busy}><UserPlus/> Criar convite</button></form>{invite && <div className="invite-token"><span>CÃƒÆ’Ã‚Â³digo do convite</span><code>{invite}</code></div>}</section></div>}
  <section className="cycle-section"><div className="section-line"><div><span>SEUS CICLOS</span><h2>CartÃƒÆ’Ã‚Âµes e vencimentos</h2></div><strong>{cards.length}</strong></div>{cards.length ? <div className="cycle-list">{cards.map(card => <article key={card.id}><span className="due-day">{String(card.dueDay).padStart(2,'0')}</span><div><strong>{card.name}</strong><small>Vencimento mensal no dia {card.dueDay}</small></div></article>)}</div> : <p className="cloud-empty">{account.role === 'master' ? 'Cadastre o primeiro cartÃƒÆ’Ã‚Â£o para liberar o envio de PDFs.' : 'O master ainda nÃƒÆ’Ã‚Â£o cadastrou cartÃƒÆ’Ã‚Âµes.'}</p>}</section>
  {account.role === 'master' && cards.length > 0 && <UploadForm cards={cards} busy={busy} fileRef={fileInput} onUpload={upload}/>}<section className="document-section"><div className="section-line"><div><span>ARQUIVO DO GRUPO</span><h2>Faturas por vencimento</h2></div><strong>{documents.length}</strong></div>{documents.length ? <div className="document-list">{documents.map(doc => <article key={doc.id}><div className="document-date"><span>{date(doc.dueDate).slice(0,5)}</span><small>{doc.dueDate.slice(0,4)}</small></div><FileText/><div><strong>{cardName(doc.cardId)}</strong><small>Vence em {date(doc.dueDate)} Ãƒâ€šÃ‚Â· {size(doc.size)}</small></div><a href={endpoint(`documents/${doc.id}`)}><Download/> Baixar PDF</a></article>)}</div> : <p className="cloud-empty">Nenhuma fatura foi enviada para este grupo.</p>}</section>
  {account.role === 'master' && <section className="member-section"><div className="section-line"><div><span>ACESSO</span><h2>Pessoas vinculadas</h2></div><strong>{members.length}</strong></div><div className="member-list">{members.map(member => <div key={member.id}><ProfilePhoto account={member}/><div><strong>{member.name}</strong><small>{member.email} Ãƒâ€šÃ‚Â· {member.role === 'master' ? 'Master' : member.active ? 'Comprador ativo' : 'Acesso revogado'}</small></div>{member.role === 'buyer' && member.active && <button onClick={async () => { if (!confirm(`Revogar o acesso de ${member.name}?`)) return; try { await api('members/revoke', json({ userId: member.id })); await refresh(account); setNotice('Acesso revogado.'); } catch (reason) { setError((reason as Error).message); } }}>Revogar</button>}</div>)}</div></section>}</section></main>;
}

function ProfilePhoto({ account }: { account: Account }) {
  async function choose(file?: File) {
    if (!file || !['image/jpeg', 'image/png', 'image/webp'].includes(file.type) || file.size > 2 * 1024 * 1024) { window.alert('Use uma foto JPEG, PNG ou WebP de atÃƒÆ’Ã‚Â© 2 MB.'); return; }
    ...
    if (!response.ok) { const data = await response.json().catch(() => null); window.alert(data?.error || 'NÃƒÆ’Ã‚Â£o foi possÃƒÆ’Ã‚Â­vel salvar a foto.'); return; }
...
function UploadForm({ cards, busy, fileRef, onUpload }: { cards: Card[]; busy: boolean; fileRef: RefObject<HTMLInputElement | null>; onUpload: (file?: File, cardId?: string, date?: string) => void }) {
  ...
  return <section className="upload-band"><div><Upload/><span><strong>Guardar nova fatura</strong><small>Escolha o cartÃƒÆ’Ã‚Â£o e informe a data real do vencimento.</small></span></div><select aria-label="CartÃƒÆ’Ã‚Â£o" value={cardId} onChange={event => setCardId(event.target.value)}>{cards.map(card => <option key={card.id} value={card.id}>{card.name} Ãƒâ€šÃ‚Â· dia {card.dueDay}</option>)}</select><input aria-label="Data real de vencimento" type="date" value={dueDate} onChange={event => setDueDate(event.target.value)}/><label className="file-choice"><input ref={fileRef} type="file" accept="application/pdf,.pdf" onChange={event => setFile(event.target.files?.[0])}/><span>{file?.name || 'Selecionar PDF'}</span></label><button disabled={busy || !file || !dueDate} onClick={() => onUpload(file, cardId, dueDate)}>{busy ? 'EnviandoÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦' : 'Guardar fatura'}</button></section>;
}
