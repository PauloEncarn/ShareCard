'use client';
import './avatar.css';
import styles from './cards-visual.module.css';
...
type SavedWorkspace = { state: AppState | null; version: number };
...
export default function CloudWorkspace({ embedded = false, section = 'management' }: { embedded?: boolean; section?: 'profile' | 'management' }) {
  ...
  const [cards, setCards] = useState<Card[]>(() => initial?.cards ?? []), [cardsLoaded, setCardsLoaded] = useState(() => Boolean(initial?.managementLoaded)), [documents, setDocuments] = useState<Document[]>(() => initial?.documents ?? []), [members, setMembers] = useState<Account[]>(() => initial?.members ?? []), [workspaceState, setWorkspaceState] = useState<AppState | null>(null), [mode, setMode] = useState<'login'|'register'>('login');
  ...
  const refresh = useCallback(async (current: Account) => {
    ...
    const [nextCards, nextDocuments, nextMembers, savedWorkspace] = await Promise.all([api<Card[]>('cards'), api<Document[]>('documents'), current.role === 'master' ? api<Account[]>('members') : Promise.resolve([]), api<SavedWorkspace>('workspace')]);
    setCards(nextCards); setCardsLoaded(true); setDocuments(nextDocuments); setMembers(nextMembers); setWorkspaceState(savedWorkspace.state);
  ...
  useEffect(() => {
    ...
    }).catch(() => { clearClientWorkspace(); setCardsLoaded(false); setError('Não foi possível carregar seus cartões agora. Atualize a página para tentar novamente.'); }).finally(() => setLoading(false));
  }, [refresh, section]);
  ...
  const selectedDocumentIds = new Set(documents.filter(document => document.cardId === selectedCardId).map(document => document.id));
  const selectedPeople = workspaceState?.people || [];
  const selectedPurchases = workspaceState?.statements.flatMap(statement => selectedDocumentIds.has(statement.storageDocumentId || '') ? statement.transactions.map(transaction => { const person = workspaceState.people.find(item => item.id === transaction.buyerId || transaction.allocations.some(allocation => allocation.personId === item.id)); return { ...transaction, person, account: person?.accountId ? members.find(member => member.id === person.accountId) : undefined }; }) : []) || [];
  ...
  <section className={`${styles.cardsSection} cycle-section`}><div className="section-line"><div><span>SEUS CARTÕES</span><h2>Cartões e compradores</h2></div><strong>{cards.length}</strong></div>{!cardsLoaded ? <p className="cloud-empty" role="status">Carregando seus cartões…</p> : cards.length ? <div className="visual-card-grid">{cards.map(card => <button key={card.id} type="button" className={`visual-credit-card ${card.name.toLowerCase().includes('nubank') ? 'nubank' : card.name.toLowerCase().includes('caixa') ? 'caixa' : 'itau'} ${selectedCardId === card.id ? 'selected' : ''}`} onClick={() => setSelectedCardId(card.id)}><span className="visual-card-bank">{card.name}</span><span className="visual-card-chip"/><span className="visual-card-number">•••• •••• •••• {card.id.slice(-4)}</span><span className="visual-card-due">VENCE DIA {String(card.dueDay).padStart(2,'0')}</span></button>)}</div> : <p className="cloud-empty">Cadastre seu primeiro cartão a partir da importação da fatura.</p>}{selectedCardId && <><button className={styles.cardBackdrop} type="button" aria-label="Fechar detalhes do cartão" onClick={() => setSelectedCardId('')}/><div className="selected-card-detail" role="dialog" aria-modal="true" aria-label="Detalhes do cartão"><button className="selected-card-close" type="button" onClick={() => setSelectedCardId('')}>Fechar</button><div><span className="eyebrow">CARTÃO SELECIONADO</span><h3>{cardName(selectedCardId)}</h3><p>{documents.filter(document => document.cardId === selectedCardId).length} faturas salvas neste cartão.</p><div className="selected-card-purchases"><strong>Compras deste cartão</strong>{selectedPurchases.length ? <ul>{selectedPurchases.map(purchase => <li key={purchase.id}><span className="card-purchase-person"><BuyerAvatar account={purchase.account} name={purchase.person?.name}/><span><b>{purchase.merchant}</b><small>{purchase.date}{purchase.person ? ` · ${purchase.person.name}` : ''}</small></span></span><b>{(purchase.cents / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</b></li>)}</ul> : <p>Nenhuma compra organizada para este cartão ainda.</p>}</div></div><div className="selected-card-buyers">{selectedPeople.map(person => <div key={person.id}><BuyerAvatar account={person.accountId ? members.find(member => member.id === person.accountId) : undefined} name={person.name}/><span>{person.name}</span></div>)}{!selectedPeople.length && <span>Nenhuma pessoa cadastrada neste espaço.</span>}</div>{account.role === 'master' && <button className="text-button" type="button" disabled={busy} onClick={() => { const card = cards.find(item => item.id === selectedCardId); if (card) void removeCard(card); }}>Excluir cartão</button>}</div></>}</section>  {account.role === 'master' && cards.length > 0 && <UploadForm cards={cards} busy={busy} fileRef={fileInput} onUpload={upload}/>}<section className="document-section"><div className="section-line"><div><span>ARQUIVO DO GRUPO</span><h2>Faturas por vencimento</h2></div><strong>{documents.length}</strong></div>{documents.length ? <div className="document-list">{documents.map(doc => <article key={doc.id}><div className="document-date"><span>{date(doc.dueDate).slice(0,5)}</span><small>{doc.dueDate.slice(0,4)}</small></div><FileText/><div><strong>{cardName(doc.cardId)}</strong><small>Vence em {date(doc.dueDate)} · {size(doc.size)}</small></div><a href={endpoint(`documents/${doc.id}`)}><Download/> Baixar PDF</a>{account.role === 'master' && <button className="text-button document-delete" type="button" disabled={busy} onClick={() => removeDocument(doc)}>Excluir</button>}</article>)}</div> : <p className="cloud-empty">Nenhuma fatura foi enviada para este grupo.</p>}</section>
...
function BuyerAvatar({ account, name }: { account?: Account; name?: string }) {
  const label = account?.name || name || 'Pessoa';
  return account?.avatarUrl ? <img className="card-buyer-avatar" src={account.avatarUrl} alt={`Foto de ${label}`}/> : <span className="card-buyer-avatar card-buyer-initial" aria-label={`${label} sem foto`}>{label.slice(0, 1).toUpperCase()}</span>;
}
