...
export default function CloudWorkspace({ embedded = false, section = 'management' }: { embedded?: boolean; section?: 'profile' | 'management' }) {
  ...
  const [cards, setCards] = useState<Card[]>(() => initial?.cards ?? []), [cardsLoaded, setCardsLoaded] = useState(() => Boolean(initial?.managementLoaded)), [documents, setDocuments] = useState<Document[]>(() => initial?.documents ?? []), [members, setMembers] = useState<Account[]>(() => initial?.members ?? []), [mode, setMode] = useState<'login'|'register'>('login');
  ...
  const refresh = useCallback(async (current: Account) => {
    ...
    setCards(nextCards); setCardsLoaded(true); setDocuments(nextDocuments); setMembers(nextMembers);
